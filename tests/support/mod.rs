...
const RUNTIME_OWNER_MARKER: &str = ".omni-test-owner-pid";

pub fn register_spawned_omni_pid(pid: Option<u32>) {
...
pub fn unregister_spawned_omni_pid(pid: Option<u32>) {
    let Some(pid) = pid else {
        return;
    };
...
pub fn cleanup_registered_omni_pids() {
    let pids: Vec<u32> = {
        let mut registry = pid_registry_lock();
...
fn start_global_watchdog() {
    thread::spawn(|| loop {
        ...
        if let Err(err) = cleanup_servers_with_missing_runtime_dir() {
            eprintln!("omni test cleanup watchdog error: {err}");
        }
    });
}
...
fn iter_worktree_server_pids() -> std::io::Result<Vec<u32>> {
    ...
    for entry in proc_entries {
        ...
        if is_test_omni_server_process(pid) {
            pids.push(pid);
        }
    }
...
fn is_test_omni_server_process(pid: u32) -> bool {
    let Some(exe_path) = proc_link_target(pid, "exe") else {
        return false;
    };

    if !is_test_omni_binary(&exe_path) {
        return false;
    }
...
fn process_runtime_dir(pid: u32) -> std::io::Result<Option<PathBuf>> {
    ...
    for entry in environ.split(|byte| *byte == 0) {
        ...
        if let Some(value) = kv.strip_prefix("OMNI_SOCKET_PATH=") {
...
fn is_test_omni_binary(path: &Path) -> bool {
    path.ends_with("target/debug/omni") && path.starts_with(current_checkout_root())
}
...
mod tests {
    ...
    fn unique_missing_runtime_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "omni-watchdog-scoping-{label}-{}-{unique}",
            std::process::id()
        ))
    }
    ...
    fn test_binary_matcher_accepts_current_checkout_debug_binary() {
        let binary = current_checkout_root().join("target/debug/omni");
        assert!(
            is_test_omni_binary(&binary),
    ...
    fn test_binary_matcher_rejects_installed_binary() {
        assert!(
            !is_test_omni_binary(Path::new("/home/can/.local/bin/omni")),
