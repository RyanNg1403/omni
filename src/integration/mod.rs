...
pub(crate) const OMNI_PANE_ID_ENV_VAR: &str = "OMNI_PANE_ID";
const PI_EXTENSION_INSTALL_NAME: &str = "omni-agent-state.ts";
const PI_EXTENSION_ASSET: &str = include_str!("assets/pi/omni-agent-state.ts");
const PI_INTEGRATION_VERSION: u32 = 2;
const OMP_EXTENSION_INSTALL_NAME: &str = "omni-omp-agent-state.ts";
const OMP_EXTENSION_ASSET: &str = include_str!("assets/omp/omni-agent-state.ts");
const OMP_INTEGRATION_VERSION: u32 = 2;
const PI_CODING_AGENT_DIR_ENV_VAR: &str = "PI_CODING_AGENT_DIR";
const CLAUDE_HOOK_INSTALL_NAME: &str = "omni-agent-state.sh";
const CLAUDE_HOOK_ASSET: &str = include_str!("assets/claude/omni-agent-state.sh");
const CLAUDE_INTEGRATION_VERSION: u32 = 4;
const CLAUDE_CONFIG_DIR_ENV_VAR: &str = "CLAUDE_CONFIG_DIR";
const CODEX_HOOK_INSTALL_NAME: &str = "omni-agent-state.sh";
const CODEX_HOOK_ASSET: &str = include_str!("assets/codex/omni-agent-state.sh");
const CODEX_INTEGRATION_VERSION: u32 = 4;
const CODEX_HOME_ENV_VAR: &str = "CODEX_HOME";
const OPENCODE_PLUGIN_INSTALL_NAME: &str = "omni-agent-state.js";
const OPENCODE_PLUGIN_ASSET: &str = include_str!("assets/opencode/omni-agent-state.js");
const OPENCODE_INTEGRATION_VERSION: u32 = 2;
const HERMES_PLUGIN_INSTALL_NAME: &str = "omni-agent-state";
...
const QODERCLI_HOOK_INSTALL_NAME: &str = "omni-agent-state.sh";
const QODERCLI_HOOK_ASSET: &str = include_str!("assets/qodercli/omni-agent-state.sh");
const QODERCLI_INTEGRATION_VERSION: u32 = 1;
const QODERCLI_CONFIG_DIR_ENV_VAR: &str = "QODER_CONFIG_DIR";
const INTEGRATION_VERSION_MARKER: &str = "OMNI_INTEGRATION_VERSION=";
...
pub(crate) fn apply_pane_env(cmd: &mut CommandBuilder, pane_id: PaneId) {
    ...
    cmd.env(OMNI_PANE_ID_ENV_VAR, format!("p_{}", pane_id.raw()));
}
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Claude => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed omni claude hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no omni claude hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Codex => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed omni codex hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no omni codex hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages.push(format!(
        ...
        crate::api::schema::IntegrationTarget::Qodercli => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed omni qodercli hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no omni qodercli hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
...
pub(crate) fn print_outdated_update_notice() -> bool {
    ...
    let targets = outdated
        ...
        .collect::<Vec<_>>();
    eprintln!(
        "installed omni integrations need updating; {}.",
...
fn remove_legacy_pi_extension_from_omp_dir(dir: &Path) -> io::Result<bool> {
    ...
    if content.contains("OMNI_INTEGRATION_ID=pi") {
...
pub(crate) fn install_qodercli() -> io::Result<QodercliInstallPaths> {
    ...
    // returns early on it (mirroring assets/claude/omni-agent-state.sh) so
...
fn update_hermes_enabled_plugin(content: &str, enabled: bool) -> String {
    ...
    if let Some(enabled_index) = enabled_index {
        ...
        if line == "enabled: []" || line == "enabled: [] # omni" {
            if enabled {
                lines[enabled_index] = "  enabled:".to_string();
                lines.insert(enabled_index + 1, "    - omni-agent-state".to_string());
            }
        ...
        match (enabled, existing_item_index) {
            ...
            (true, None) => lines.insert(list_start, "    - omni-agent-state".to_string()),
            (false, Some(index)) => {
                lines.remove(index);
            }
        }
    ...
    if enabled {
        ...
        lines.insert(plugins_index + 2, "    - omni-agent-state".to_string());
        return join_yaml_lines(lines, trailing_newline);
    }
...
mod tests {
    ...
    fn integration_recommendation_installs_available_or_outdated_targets() {
        let mut recommendation = IntegrationRecommendation {
            ...
            command: "claude",
            available: false,
            path: PathBuf::from("/tmp/omni-agent-state.sh"),
            state: IntegrationStatusKind::NotInstalled,
        };
    ...
    #[test]
    fn install_omp_preserves_non_omni_file_with_pi_install_name() {
        let _lock = integration_env_lock();
        let base = unique_base();
        let home = base.join("home");
        let ext_dir = home.join(".omp/agent/extensions");
    ...
    fn outdated_integrations_treat_missing_version_marker_as_legacy() {
        ...
        fs::write(&extension_path, "// installed by omni\n").unwrap();
        std::env::set_var("HOME", &home);
    ...
    fn claude_v1_integration_status_is_outdated() {
        ...
        fs::write(
            &hook_path,
            "#!/bin/sh\n# OMNI_INTEGRATION_ID=claude\n# OMNI_INTEGRATION_VERSION=1\n",
        )
        .unwrap();
        std::env::set_var("HOME", &home);
    ...
    #[test]
    fn uninstall_claude_removes_omni_hooks_and_preserves_others() {
        let _lock = integration_env_lock();
        let base = unique_base();
        let home = base.join("home");
        let claude_dir = home.join(".claude");
    ...
    #[test]
    fn uninstall_codex_removes_omni_hooks_and_leaves_config_alone() {
        let _lock = integration_env_lock();
        let base = unique_base();
        let home = base.join("home");
        let codex_dir = home.join(".codex");
    ...
    fn install_hermes_is_idempotent_for_enabled_entry() {
        ...
        assert_eq!(config.matches("omni-agent-state").count(), 1);
    ...
    fn uninstall_hermes_removes_plugin_and_enabled_entry() {
        ...
        assert!(!config.contains("omni-agent-state"));
    ...
    fn bundled_integration_assets_report_session_refs() {
        ...
        assert!(CODEX_HOOK_ASSET.contains("OMNI_HOOK_INPUT_FILE"));
        ...
        assert!(QODERCLI_HOOK_ASSET.contains("OMNI_HOOK_INPUT_FILE"));
    ...
    #[test]
    fn uninstall_qodercli_removes_omni_hooks_and_preserves_others() {
        let _lock = integration_env_lock();
        let base = unique_base();
        let qoder_dir = base.join(".qoder");
        fs::create_dir_all(&qoder_dir).unwrap();
