"""Hermes plugin installed by Omni to report agent lifecycle state."""

# OMNI_INTEGRATION_ID=hermes
...
def _base_params() -> tuple[str, str] | None:
    if os.environ.get("OMNI_ENV") != "1":
        return None
    pane_id = os.environ.get("OMNI_PANE_ID", "").strip()
    socket_path = os.environ.get("OMNI_SOCKET_PATH", "").strip()
