...
# OMNI_INTEGRATION_ID=qodercli
# OMNI_INTEGRATION_VERSION=1
#
# Reports qodercli agent state changes to omni. Registered as a Command hook
# in ~/.qoder/settings.json by `omni integration install qodercli` and
...
# `assets/claude/omni-agent-state.sh` already consumes claude code's stdin
# payload. No environment variable is consulted for the event identity.

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/omni-qodercli-hook.XXXXXX")" || exit 0
...
[ "${OMNI_ENV:-}" = "1" ] || exit 0
[ -n "${OMNI_SOCKET_PATH:-}" ] || exit 0
[ -n "${OMNI_PANE_ID:-}" ] || exit 0
command -v python3 >/dev/null 2>&1 || exit 0

OMNI_ACTION="$action" OMNI_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
source = "omni:qodercli"
action = os.environ.get("OMNI_ACTION", "")
pane_id = os.environ.get("OMNI_PANE_ID")
socket_path = os.environ.get("OMNI_SOCKET_PATH")
hook_input_file = os.environ.get("OMNI_HOOK_INPUT_FILE")
