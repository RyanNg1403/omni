...
pub(crate) fn init_file_logging(file_name: &str) {
    ...
    let filter =
        EnvFilter::try_from_env("OMNI_LOG").unwrap_or_else(|_| EnvFilter::new("omni=info"));
...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus omni-client.log, omni-server.log)",
        dir.join("omni.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("omni.log")
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "omni.log", 8, 0).unwrap();
        {
