...
mod tests {
    ...
    fn temp_git_repo(branch: &str) -> std::path::PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("unix time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("omni-ui-test-{unique}"));
    ...
    fn release_notes_inline_code_spans_are_styled_without_backticks() {
        ...
        let lines = release_notes_lines("- `omni pane run ...` now works", &palette);

        assert_eq!(lines.len(), 1);
        assert_eq!(line_text(&lines[0].1), " • omni pane run ... now works");
        assert_eq!(lines[0].1.spans[1].content.as_ref(), "omni pane run ...");
    ...
    fn release_notes_preview_lines_show_update_steps() {
        let palette = Palette::catppuccin();
        let lines = release_notes_preview_lines("0.5.0", "omni update", &palette);

        assert_eq!(lines.len(), 2);
        assert_eq!(line_text(&lines[0]), "● update ready");
        assert_eq!(
            line_text(&lines[1]),
            "detach from this session, then run omni update in your shell"
        );
