...
//!
//! The client:
//! - Connects to `omni-client.sock`, sends Hello with terminal size and protocol version
...
fn requested_render_encoding() -> RenderEncoding {
    match std::env::var("OMNI_RENDER_ENCODING").ok().as_deref() {
...
fn run_client_with_mode(
    ...
    if let Some((terminal_id, takeover)) = attach_request {
        ...
        if let Err(err) = write_to_server(&mut stream, &attach) {
            eprintln!("omni: failed to request terminal attach: {err}");
            std::process::exit(1);
        }
    }
    ...
    .map_err(|err| {
        eprintln!("omni: failed to set up terminal: {err}");
        err
    })?;
...
fn init_logging() {
    crate::logging::init_file_logging("omni-client.log");
}
...
mod tests {
    ...
    #[test]
    fn kitty_graphics_image_id_parser_tracks_omni_ids_only() {
    ...
    fn client_error_display_detached_default_session_reattach_hint() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Run `omni` to reattach"),
    ...
    fn client_error_display_detached_named_session_reattach_hint() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Run `omni session attach work` to reattach"),
    ...
    fn client_error_display_detached_remote_reattach_hint_takes_precedence() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Run `omni --remote host --session work` to reattach"),
    ...
    fn reload_local_client_config_refreshes_redraw_on_focus_gained() {
        ...
        let path = std::env::temp_dir().join(format!(
            "omni-client-config-reload-{}-{}.toml",
            std::process::id(),
