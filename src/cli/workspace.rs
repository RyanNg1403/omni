...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: omni workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: omni workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: omni workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: omni workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: omni workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: omni workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: omni workspace close <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: omni workspace close <workspace_id>");
...
fn print_workspace_help() {
    eprintln!("omni workspace commands:");
    eprintln!("  omni workspace list");
    eprintln!("  omni workspace create [--cwd PATH] [--label TEXT] [--focus] [--no-focus]");
    eprintln!("  omni workspace get <workspace_id>");
    eprintln!("  omni workspace focus <workspace_id>");
    eprintln!("  omni workspace rename <workspace_id> <label>");
    eprintln!("  omni workspace close <workspace_id>");
}
