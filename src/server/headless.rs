//! Headless server mode — runs the omni event loop without a real terminal.
//!
//! The server:
//! - Does not enter raw mode or read stdin
//! - Creates and listens on both `omni.sock` (existing JSON API) and
//!   `omni-client.sock` (new binary protocol)
...
/// The headless server — runs the omni event loop without a real terminal.
...
impl HeadlessServer {
    ...
    fn perform_live_handoff(
        ...
        if pane_by_terminal.len() > crate::server::handoff::MAX_FDS_PER_HANDOFF {
            ...
            return Err(io::Error::new(
                ...
                format!(
                    "live handoff supports at most {} panes in one update; close panes or restart omni normally",
    ...
    fn handle_api_request_with_shutdown_check(&mut self, msg: api::ApiRequestMessage) -> bool {
        ...
        // Omni delivery renders the toast in-frame and must not ask clients to
...
pub fn run_server() -> io::Result<()> {
    ...
    let _api_server = match api::start_server(api_tx.clone(), event_hub.clone()) {
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: omni server is already running");
...
fn run_handoff_import_server(socket_path: &Path, token: &str) -> io::Result<()> {
    ...
    let result = rt.block_on(async {
        ...
        if std::env::var("OMNI_TEST_HANDOFF_IMPORT_FAIL").as_deref() == Ok("after_restored") {
...
fn print_ready_message(api_socket: &Path, client_socket: &Path) {
    eprintln!("omni server running; you can use any omni CLI command in another terminal.");
...
fn init_logging() {
    crate::logging::init_file_logging("omni-server.log");
}
...
"#,
        ...
        assert!(content.contains("delivery = \"omni\""));
    ...
    fn invalid_server_keybindings_do_not_cache_local_keybindings_after_settings_save() {
        let path = std::env::temp_dir().join(format!(
            "omni-headless-invalid-settings-{}-{}.toml",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map(|d| d.as_nanos())
...
"#,
    ...
    fn headless_scheduled_tasks_expire_agent_metadata() {
        ...
        assert!(
            server.handle_internal_event_with_forwarding(AppEvent::HookMetadataReported {
                ...
                applies_to_source: Some("omni:pi".into()),
                title: None,
                display_agent: None,
    ...
    #[test]
    fn omni_toast_delivery_keeps_toast_in_frame_without_client_notify() {
        let mut server = test_headless_server();
        let (client_tx, client_control_rx, _client_rx) = test_client_writer();
        ...
        server.foreground_client_id = Some(1);
        server.app.state.toast_config.delivery = crate::config::ToastDelivery::Omni;
        ...
        assert!(
            client_control_rx
                .recv_timeout(Duration::from_millis(50))
                .is_err(),
            "omni delivery should render in-frame instead of forwarding a client-local notification"
        );
    }
    ...
    fn system_toast_delivery_forwards_system_notify_kind() {
        ...
        match read_server_message(
            ...
            ServerMessage::Notify { kind, message } => {
                ...
                assert_eq!(message, "v9.9.9 available: detach, then run `omni update`");
            }
