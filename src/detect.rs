...
pub struct AgentDetection {
    ...
    /// This lets Omni recover from integrations that miss an interrupt/stop
...
mod tests {
    ...
    fn claude_bash_permission_modal_is_visible_blocker() {
        let screen = "● Bash(mkdir -p /tmp/omni-claude-detector-test && for i in 1 2 3; do dd if=/dev/urandom)\n  ⎿  Waiting…\n\n────────────────────────\n Bash command\n\n   mkdir -p /tmp/omni-claude-detector-test && ls -la /tmp/omni-claude-detector-test\n   Create random files in temporary detector directory\n\n Contains expansion\n\n Do you want to proceed?\n ❯ 1. Yes\n   2. No\n\n Esc to cancel · Tab to amend · ctrl+e to explain";
    ...
    fn claude_cropped_bash_permission_modal_is_visible_blocker() {
        let screen = "● Bash(mkdir -p /tmp/omni-claude-detector-test && ls -la /tmp/omni-claude-detector-test)\n  ⎿  Waiting…\n\nDo you want to proceed?\n❯ 1. Yes\n  2. No";
        let detection = detect_agent(Some(Agent::Claude), screen);
...
╰───────────────────────────────────────────────────────────────────╯";
    ...
    fn grok_blocked_on_permission_prompt() {
        let screen = "Show recent commit history for analysis\n\
                      git -C /home/can/Projects/omni log --oneline --decorate -n 12\n\
    ...
    fn grok_working_on_tool_spinner() {
        let screen = "⠼ Run git -C /home/can/Projects/omni log --oneline 1.0s";
        assert_eq!(detect_state(Some(Agent::Grok), screen), AgentState::Working);
    }
    ...
    fn hermes_blocked_on_dangerous_command_prompt() {
        let screen = "╭────────────────────────────────────────────────────────────╮\n│ ⚠️  Dangerous Command                                      │\n│ mkdir -p /tmp/omni-hermes-block-test/subdir && touch      │\n│ ❯ 1. Allow once                                            │\n│   2. Allow for this session                                │\n│   3. Add to permanent allowlist                            │\n│   4. Deny                                                  │\n│   5. Show full command                                     │\n╰────────────────────────────────────────────────────────────╯\n  ↑/↓ to select, Enter to confirm\n⚠ ❯";
        assert_eq!(
            detect_state(Some(Agent::Hermes), screen),
            AgentState::Blocked
        );
