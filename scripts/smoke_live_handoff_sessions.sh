...
OMNI_BIN="${OMNI_BIN:-$ROOT/target/debug/omni}"
BASE="${BASE:-$(mktemp -d /tmp/omni-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_omni "$session" server stop >/dev/null 2>&1 || true
  done
...
run_omni() {
  ...
  env -u OMNI_SOCKET_PATH \
    -u OMNI_CLIENT_SOCKET_PATH \
    -u OMNI_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$OMNI_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/omni-dev' "$CONFIG_HOME"
  else
    printf '%s/omni-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/omni.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/omni-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/omni-dev/omni.sock | "$CONFIG_HOME"/omni-dev/sessions/*/omni.sock)
      ;;
    *)
...
echo "using omni: $OMNI_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/omni-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/omni-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_omni "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
